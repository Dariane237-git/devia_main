            ...
            <form action="{{ route('materiels.update', $materiel->id) }}" method="POST">
                ...
                <div class="form-grid">
                        ...
                        <div class="form-group">
                            ...
                            <select name="id_client" class="form-select" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" {{ old('id_client', $materiel->id_client) == $client->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-span-2">
                        ...
                        <input type="text" name="nom" value="{{ old('nom', $materiel->nom) }}" class="form-input" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="date_achat" value="{{ old('date_achat', $materiel->date_achat ? \Carbon\Carbon::parse($materiel->date_achat)->format('Y-m-d') : '') }}" class="form-input">
                        @error('date_achat') <span class="error-text">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Marque</label>
                        <input type="text" name="marque" value="{{ old('marque', $materiel->marque) }}" class="form-input">
                        @error('marque') <span class="error-text">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group">
                        <label class="form-label">Modèle</label>
                        <input type="text" name="modele" value="{{ old('modele', $materiel->modele) }}" class="form-input">
                        @error('modele') <span class="error-text">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group col-span-2">
                        <label class="form-label">Garantie</label>
                        <input type="text" name="garantie" value="{{ old('garantie', $materiel->garantie) }}" class="form-input">
